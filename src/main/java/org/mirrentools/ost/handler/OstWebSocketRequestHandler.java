...
public interface OstWebSocketRequestHandler {
    ...
    static void requestAbs(HttpClient httpClient, OstRequestOptions options, Handler<AsyncResult<WebSocket>> handler) {
        ...
        httpClient.webSocketAbs(options.getUrl(), header, WebsocketVersion.valueOf(options.getWebSocketVersion()), options.getSubProtocols(), handler);
    }
     ...
     * @param options          请求的配置
     * @param succeededHandler 请求成功
     * @param exceptionHandler 请求失败
     */
     ...
     * @param httpClient        Http客户端
     * @param options           请求的配置
     * @param connectionHandler 连接建立处理器
     * @param succeededHandler  请求成功
     * @param exceptionHandler  请求失败
     */
    static void requestAbs(HttpClient httpClient, OstRequestOptions options, Handler<HttpConnection> connectionHandler, Handler<WebSocket> succeededHandler, Handler<Throwable> exceptionHandler) {
        try {
            MultiMap header = MultiMap.caseInsensitiveMultiMap();
            options.getHeaders().forEach(header::add);
            requestAbs(httpClient, options.getUrl(),header, WebsocketVersion.valueOf(options.getWebSocketVersion()), options.getSubProtocols(), connectionHandler, succeededHandler, exceptionHandler);
        } catch (Exception e) {
            if (exceptionHandler != null) {
                exceptionHandler.handle(e);
            }
